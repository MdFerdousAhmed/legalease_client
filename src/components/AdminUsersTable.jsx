...
export default function AdminUsersTable({ users }) {
  ...
  const formatDate = (dateObj) => {
    ...
    // 1. Check if it's a raw MongoDB $date object
    if (dateObj.$date) {
      date = new Date(dateObj.$date);
    }
    // 2. Check if it's already a standard JS Date object or an ISO string
    else if (dateObj instanceof Date || typeof dateObj === 'string') {
  ...
  return (
    <div className="relative w-full">
      <div className="w-full  border border-zinc-800 rounded-xl overflow-hidden shadow-2xl font-sans">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-sm text-zinc-400">

            {/* Header */}
            <thead>
              <tr className="border-b bg-[#1e1e1e] border-zinc-800 text-zinc-400 font-medium select-none">
            ...
            <tbody className="divide-y divide-zinc-800/60 bg-[#1e1e1e]">
              {users.map((user) => {
                ...
                const userRole = user.userRole?.toLowerCase() || 'user';
                  ...
                  <tr key={userId} className="hover:bg-zinc-900/40 transition-colors duration-150">
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap text-xs font-medium">
                      <div className="flex items-center justify-end gap-4">
                        ...
                        {userRole !== 'lawyer' && (
                          <button
                            onClick={() => initiateRoleChange(userId, user.name, 'lawyer')}
                            className="text-zinc-400 hover:text-white transition-colors"
                          >
                            Make lawyer
                          </button>
                        ...
                        {userRole !== 'user' && (
                          <button
                            onClick={() => initiateRoleChange(userId, user.name, 'user')}
                            className="text-zinc-400 hover:text-white transition-colors"
                          >
                            Make user
                          </button>
        ...
        {/* Pagination Footer */}
        <div className="flex items-center bg-[#1e1e1e]  justify-between px-6 py-4 border-t border-zinc-800 text-xs text-zinc-500 select-none">
          <div>
